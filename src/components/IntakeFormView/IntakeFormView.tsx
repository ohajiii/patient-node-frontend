...
export default function IntakeFormView(props: { form: IntakeFormResponseDto }) {
  ...
  return (
    <div className="bg-white shadow border-l-4 border-primary rounded-lg p-6 mt-6">
      <h2 className="text-xl font-semibold text-gray-800 mb-4">Intake Form Details</h2>
      ...
      <div className="space-y-6 text-gray-700">
        ...
        <div>
          ...
          <p className="mt-2">
            <span className="font-medium">Medication:</span> {form.takesMedication ? "Yes" : "No"}
          </p>
        ...
        <div>
          <h3 className="text-lg font-medium text-gray-900 mb-2">Other Conditions</h3>
          <p><span className="font-medium">Allergies:</span> {form.hasAllergies ? "Yes" : "No"}</p>
          {form.allergyDetails && <p><span className="font-medium">Details:</span> {form.allergyDetails}</p>}

          <p className="mt-2"><span className="font-medium">Surgeries:</span> {form.hasSurgeries ? "Yes" : "No"}</p>
          {form.surgeryDetails && <p><span className="font-medium">Details:</span> {form.surgeryDetails}</p>}
        </div>

        <div>
          <h3 className="text-lg font-medium text-gray-900 mb-2">Lifestyle</h3>
          <p><span className="font-medium">Smokes:</span> {form.smokes ? "Yes" : "No"}</p>

          <p className="mt-2"><span className="font-medium">Alcohol Use:</span> {form.drinksAlcohol ? "Yes" : "No"}</p>
          {form.alcoholFrequency && <p><span className="font-medium">Frequency:</span> {form.alcoholFrequency}</p>}

          <p className="mt-2">
            <span className="font-medium">Exercises Regularly:</span> {form.exercisesRegularly ? "Yes" : "No"}
          </p>
          {form.exerciseFrequency && <p><span className="font-medium">Frequency:</span> {form.exerciseFrequency}</p>}
        </div>

        <div>
          <h3 className="text-lg font-medium text-gray-900 mb-2">Primary Complaint</h3>
          <p><span className="font-medium">Complaint:</span> {form.primaryComplaint}</p>
          <p><span className="font-medium">Start Date:</span> {form.symptomStart}</p>
